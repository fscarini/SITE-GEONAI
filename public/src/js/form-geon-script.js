...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentStep = 1;
  const questionSteps = Array.from(steps).filter(
    (step) => !step.classList.contains("success-step")
  );
  ...
  // --- MUDANÇA: Adiciona suporte à tecla 'Enter' para avançar e enviar ---
  document.addEventListener("keydown", (e) => {
    // 1. Verifica se a tecla pressionada é 'Enter' (código 13 ou 'Enter')
    if (e.key === "Enter") {
      ...
      // 4. Se o foco estiver em um <textarea>, permite o Enter para nova linha.
  ...
  function updateStep() {
    ...
    if (activeStepElement) {
      ...
      const inputField = activeStepElement.querySelector(
    ...
    progressBar.style.width = `${
      currentStep > totalQuestionSteps ? 100 : progress
    }%`;

    if (currentStep > totalQuestionSteps) {
      // Se for o Passo 9 (Sucesso)
    ...
    if (!inputField || !inputField.hasAttribute("required")) {
      if (inputField && inputField.id === "website") {
        ...
        if (value !== "" && !value.includes(".")) {
          showFloatingError(
            "Por favor, insira um website válido (ex: site.com)"
          );
          return false;
        }
      }
  ...
  form.addEventListener("submit", async function (e) {
    ...
    if (!validateStep()) {
      // Valida o último campo (Problema)
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 || response.status === 401) {
          throw new Error(
            errorData.error || "Código inválido. Tente novamente."
          );
        }
